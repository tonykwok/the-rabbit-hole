 ...
 * {@link #paintDropTarget(DropTargetEvent, int, Point)}.
 * </ul>
 * 
 * The method {@link #getDropAction(DropTargetEvent)} follows these steps to
 * determine the appropriate action (if any).  
 * <ul>
 * <li>{@link #isSupported(List)} determines if there are any supported flavors
 * <li>{@link #getDropActionsForFlavor(List)} reduces the supported actions
 * based on available flavors.  For instance, a text field for file paths
 * might support {@link DnDConstants#ACTION_COPY_OR_MOVE} on a plain string, 
 * but {@link DnDConstants#ACTION_LINK} might be the only action supported on 
 * a file.
...
// constructor, and provide defaults for standard components?  this would
// handle the highlight/clear logic automatically (although the highlighter
// could probably handle that itself each time it's drawn).
...
public abstract class DropHandler implements DropTargetListener {
    ...
    public DropHandler(final Component c, int acceptedActions, 
        ...
        // NOTE: look into making use of the existing 
        // Transferable.SwingDropTarget on JComponent instances; 
        // we can add self as a listener
        // We don't, however, want to make use of the default drag handler,
        // since it doesn't refuse invalid user actions or automatically
        // use a non-default action if appropriate.
    ...
    protected int getDropAction(DropTargetEvent e) {
        ...
        if (isSupported(flavors)) {
            ...
            int availableActions = getDropActionsForFlavor(flavors);
     ...
     * {@link #getDropActionsForFlavor(List)} instead.
     * @see #getDropActionsForFlavor(List)
     * @see #canDrop(DropTargetEvent, int, Point)
     */
    protected int getDropAction(DropTargetEvent e, int currentAction, 
        ...
        if ((currentAction & acceptedActions) == DragHandler.NONE
            ...
            Log.debug("drop action relaxed to " 
                      + DragHandler.actionString(action)
                      + " from " + DragHandler.actionString(currentAction));
     ...
     * instance of {@link AbstractComponentDecorator} and moving it according
     * to the intended drop location.
...
class Log {
    ...
    public static boolean isClassDebugEnabled(Class c) { return false; }
}